export default class GLUtils {
  ...
  createProgram(gl, vertex, fragment, attribs) {
    ...
    for (var i = 0; i < unf_length; ++i) {
      ...
      var uobj = {
        ...
        loc  : location,
        type : u.type,
        set  : this.make_unf_set(gl, u.name, location, u.type),
        setv : this.make_unf_setv(gl, u.name, location, u.type),
      };
  ...
  make_unf_set(gl, name, location, type) {
    if (type == gl.FLOAT) {
      return function(v0) {
  ...
  make_unf_setv(gl, name, location, type) {
    if (type === gl.FLOAT) {
      return function(value) {
  ...
  initAttribs(gl, attribs, offset = 0) {
    var stride = 0;
  ...
  loadTexture(gl, filename, format = gl.RGBA, generate_mipmap = true, nearest = false, repeat = false) {
    var tex = gl.createTexture();
    var image = new Image();
    image.onload = () => {
      this.setTexImage(gl, image, tex, format, generate_mipmap, nearest, repeat);
    };
    image.src = filename;
  ...
  setTexImage(gl, image, tex, format, generate_mipmap, nearest_filtering, repeat_uv) {
  ...
  colorFromString(string, fallback_value = [0, 0, 0]) {
